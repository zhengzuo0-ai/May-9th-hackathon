...
export function buildEvidenceExtractionPrompt(input: EvidenceExtractionInput) {
  return [
    "Extract source-backed exploration evidence for the selected AOI.",
    ...
    "Prefer focused nearby evidence within the AOI district. Regional public-company comparables may come from beyond the immediate AOI, but distant analogues are context, not direct AOI evidence.",
    ...
    "Rank evidence as: direct AOI evidence > nearby same-trend public-company / geology / geochemistry / geophysics / ASM evidence > nearby same-country context > distant unrelated analogues.",
    "Prefer concrete facts from sourcePacks. Use braveResults only as weak discovery context.",
    "",
    `AOI: ${JSON.stringify(input.concession ?? null)}`,
...
export function buildCommitteePrompt(input: CommitteeInput) {
  return [
    "Write an expert-review route in markdown for a Côte d'Ivoire AOI intelligence screen.",
    ...
    "Do not treat proximity alone as proof of mineralization. Separate direct AOI evidence from regional public-company comparables, and downgrade distant analogues unless geology, structure, or method relevance is clear.",
    "Give extra weight when public-company projects, research papers, ASM workings, or geophysical / geochemical anomalies sit on the same shear zone, structural corridor, greenstone belt, or mineralized trend as the AOI.",
...
export function buildFollowUpPrompt(input: FollowUpInput) {
  return [
    "Answer the user's follow-up for an AOI intelligence and expert collaboration workflow.",
